...
public class Clock_BY : MonoBehaviour
{
    ...
    public GameObject daypast2;
    public GameObject daypast0;
    //-- time speed factor
    public float clockSpeed = 2.0f;     // 1.0f = realtime, < 1.0f = slower, > 1.0f = faster

    //-- internal vars
    float msecs = 0;
    private static int times = 24*2;//hours
    ...
    void Update()
    {
        ...
        if (times > 0)
        {
            ...
            if (minutes < 0)
            {
                ...
                times--;
                if (hour == 0)
                    daypast2.SetActive(false);
                else if(hour ==-24)
                    daypast.SetActive(false);
                //else if(hour==-36)
                //    daypast0.SetActive(false);
            }
        }
        else if (times == 0)
        {
